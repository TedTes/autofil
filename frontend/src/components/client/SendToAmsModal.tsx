...
export default function SendToAmsModal({
  ...
  const selectedProviderIsAmsReady = Boolean(selectedConnection?.provider.endsWith('_ams_ready'))
  const requiresTargetClient = Boolean(selectedProvider?.capabilities.requiresTargetClient)
  const showTargetClientSearch = Boolean(
    requiresTargetClient || selectedProvider?.capabilities.supportsClientSearch
  )
  const sendNoun = selectedProviderIsAmsReady ? 'AMS-ready package' : 'AMS send'
      ...
      setMessage(
        job.status === 'succeeded'
          ? selectedProviderIsAmsReady
            ? 'AMS-ready package sent to the workflow.'
            : 'Submission sent to AMS.'
          : `${sendNoun} job finished with issues. Review the job result below.`
      )
    } catch (err) {
        ...
        <div className="flex items-start justify-between gap-4 border-b border-gray-200 px-5 py-4">
          <div>
            <h2 className="text-lg font-semibold text-gray-950">
              {selectedProviderIsAmsReady ? 'Send AMS-ready package' : 'Send to AMS'}
            </h2>
            <p className="mt-1 text-sm text-gray-500">
              Prepare {submissionName || 'this submission'} for a native AMS connection or a validation webhook workflow.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="mb-5 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                {amsConnections.length} saved destination{amsConnections.length === 1 ? '' : 's'} across {providers.length} providers.
              </p>
            </div>
            <button
              type="button"
          ...
          {isLoading ? (
            ...
            <div className="rounded-lg border border-dashed border-gray-300 px-4 py-8 text-center">
              <p className="text-sm font-semibold text-gray-900">No AMS-ready workflows yet</p>
              <p className="mt-1 text-xs text-gray-500">
                Add a HawkSoft, AMS360, or Applied Epic-ready webhook in Settings.
              </p>
            </div>
          ) : (
          ...
          {selectedConnection && (
            <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3">
              ...
              <p className="mt-1 text-xs text-blue-700">
                {selectedProviderIsAmsReady
                  ? `${selectedConnection.name} will receive an AMS-ready webhook payload for validation. This does not write directly into the native AMS.`
                  : `${selectedConnection.name} will receive the reviewed canonical submission after target and preview steps.`}
              </p>
            </div>
          )}

          {selectedConnection && showTargetClientSearch && (
            ...
            <div className="mt-5 rounded-lg border border-gray-200 bg-white p-4">
              <div className="mb-3 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {selectedProviderIsAmsReady
                      ? 'Generated from canonical submission data and AMS-ready webhook capabilities.'
