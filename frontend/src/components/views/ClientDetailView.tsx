          ...
          <div className="border-t border-gray-200 bg-white">
              ...
              <div className="px-3 py-2 border-b border-gray-100">
                ...
                <div className="mt-2">
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      onFillPackage?.(pkg.submission_id)
                    }}
                    disabled={isFilling || totalInputs === 0}
                  ...
                  >
                    {isFilling ? (
                      ...
                      <>
                        ...
                        {selectedInputFilenames.length > 0
                          ? `Fill with ${selectedInputFilenames.length} selected`
            ...
            {pkg.outputs && pkg.outputs.length > 0 && (
              <div className="px-3 py-2">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs font-semibold text-gray-700">Outputs</h4>
                  <span className="text-xs text-gray-500">
                    {selectedOutputFilenames.length > 0 && `${selectedOutputFilenames.length} selected`}
                  </span>
                </div>

                {/* Vertical list of output files with individual selection */}
                <div className="space-y-1.5">
                  {pkg.outputs.map((output) => {
                    const isSelected = selectedOutputFilenames.includes(output.filename)
                    return (
                      <div
                        key={output.filename}
                        className={`flex items-center gap-2 p-2 rounded-lg border transition-colors ${
                      ...
                      >
                        ...
                        >
                          <input
                            ...
                            checked={isSelected}
                            onChange={() => {
                              onToggleOutput?.(pkg.submission_id, output.filename)
                            }}
                            className="w-3.5 h-3.5 text-green-600 border-gray-300 rounded focus:ring-green-500 cursor-pointer"
                          />
                        </label>
                        <div className="flex-shrink-0">{getFileIcon(output.filename)}</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-gray-900 truncate">
                            {output.filename}
                          </p>
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            onDownloadFile?.(pkg.submission_id, output.filename)
                          }}
                          className="p-1 rounded hover:bg-gray-200 text-gray-600 hover:text-gray-900 transition-colors flex-shrink-0"
                          title="Download"
                        >
                ...
                <div className="mt-2">
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      onMergePackage?.(pkg.submission_id)
                    }}
                    disabled={isMerging || selectedOutputFilenames.length === 0}
